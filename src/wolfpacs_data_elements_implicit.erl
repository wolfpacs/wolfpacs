%%%-------------------------------------------------------------------
%% @doc Data Elements (Implicit Little).
%%
%% @end
%%%-------------------------------------------------------------------

-module(wolfpacs_data_elements_implicit).
-export([encode_map/1,
	 encode_list/1,
	 decode/1]).
...
encode([{{G, E}, Data}|Rest], Acc) ->
    ...
    Encoded = wolfpacs_data_element_implicit:encode(G, E, VR, Data),
...
decode(Data, []) ->
    case wolfpacs_data_element_implicit:decode(Data) of
	{error, _} ->
	    {error, Data};
	{ok, Res, Rest} ->
	    decode(Rest, [Res])
...
encode_list_test() ->
    UID = <<"1.2.3.4">>,
    Items = [{{?CMD, ?UID}, UID}],
    Correct = wolfpacs_data_element_implicit:encode(?CMD, ?UID, UID),
